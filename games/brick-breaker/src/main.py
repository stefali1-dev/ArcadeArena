...
print("\033[32m🟢 Start moving the ball with the SPACE KEY \033[0m")
print("\033[38;5;214m🟠 Move the bar using the LEFT KEY ◀️  and RIGHT KEY ▶️ \033[0m")
print("\033[31m🔴 Press the \"ESCAPE\" KEY or the \"Q\" KEY on the Brick Breaker window anytime to end the game! \033[0m")
print("")
...
class Game(tk.Canvas):
    ...
    def nextFrame(self):
        ...
        if not(self.textDisplayed):
            if self.won:
                self.displayText("WELL DONE, YOU WON!", callback = lambda: self.level(self.levelNum+1))
            elif self.losed:
                self.displayText("GAME OVER!", callback = lambda: self.level(self.levelNum))
...
def eventsPress(event):
    ...
    elif event.keysym == "q" or event.keysym == "Q" or event.keysym == "Escape":
        sys.exit()
...
window.bind("<Key>", eventsPress)
window.bind("<KeyRelease>", eventsRelease)
